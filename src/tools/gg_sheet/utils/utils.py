...
from src.tools.gg_sheet.utils.string_similarity import jaro_winkler_similarity
...
def filter_transaction(transaction: List[List[str]],
    ...
    for index, row in enumerate(transaction[1:], start = 1):
        ...
        # "Mô tả" column
        if description and not filter_description(description.lower(), row[3].lower()):
            continue
...
def filter_description(description: str, query: str, threshold: float = 0.85) -> bool:
    score = jaro_winkler_similarity(description, query)
